<script>
    ...
    function titleFormatter(value, row, index) {
        return row.title.substring(0, 30) +
            (row.recommended ? '<span class="badge badge-default pull-right"> 推荐</span>' : '')
    }

    function actionFormatter(value, row, index) {

        //管理
        var html = '<button class="btn btn-primary btn-xs margin-r-5 manage" data-toggle="tooltip" data-placement="top" title="管理"><i class="fa fa-cog"></i></button>';
        ...
        //推荐
        html += '<button class="btn btn-primary btn-xs margin-r-5 recommend" data-toggle="tooltip" data-placement="top" title="' + (row.recommended ? '取消推荐' : '推荐') + '"><i class="fa ' + (row.recommended ? 'fa-chevron-circle-down' : 'fa-chevron-circle-up') + '"></i></button>';
    ...
    window.actionEvents = {
        'click .edit': function (e, value, row, index) {
            ...
            window.location.href = '<?php echo e($base_url); ?>/' + row.id + '/manage';
        },

        'click .top': function (e, value, row, index) {
            $.ajax({
                url: '/admin/courses/' + row.id + '/top',
                type: 'post',
        ...
        'click .recommend': function (e, value, row, index) {
            $.ajax({
                url: '/admin/courses/' + row.id + '/recommend',
                type: 'post',
                data: {'_token': '<?php echo e(csrf_token()); ?>'},
                success: function (data) {
                    $('#table').bootstrapTable('refresh');
        ...
        'click .comment': function (e, value, row, index) {
            ...
            var url = '/admin/courses/comments/' + row.id;
            $.ajax({
                url: url,
                type: "get",
                data: {'_token': '<?php echo e(csrf_token()); ?>'},
