<script>
    ...
    $('#table_field').bootstrapTable({
        onEditableSave: function (field, row, old, $el) {
            $.ajax({
                ...
                method: 'post',
                data: {'_token': '<?php echo e(csrf_token()); ?>', 'index': row.index},
    ...
    $('#table_column').bootstrapTable({
        onEditableSave: function (field, row, old, $el) {
            $.ajax({
                url: "/admin/modules/fields/" + row.id + '/save',
                method: 'post',
                data: {'_token': '<?php echo e(csrf_token()); ?>', 'column_index': row.column_index, 'column_width': row.column_width},
                success: function (data, status) {
                },
                error: function (data) {
                    return toast('warning', '保存失败');
    ...
    $('#table_editor').bootstrapTable({
        onEditableSave: function (field, row, old, $el) {
            $.ajax({
                url: "/admin/modules/fields/" + row.id + '/save',
                method: 'post',
                data: {'_token': '<?php echo e(csrf_token()); ?>', 'editor_index': row.editor_index, 'editor_rows': row.editor_rows, 'editor_column': row.editor_columns},
                success: function (data, status) {
                },
                error: function (data) {
                    return toast('warning', '保存失败');
    ...
    $('#btn_create').click(function () {
        ...
        $('#editor_rows').val(1);
        $('#editor_group').val('<?php echo e(reset($groups)); ?>');
        $('#editor_index').val(0);
    });
