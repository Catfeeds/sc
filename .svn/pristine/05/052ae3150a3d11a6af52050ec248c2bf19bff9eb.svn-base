...
class Image extends BaseModule
{
    ...
    const MODEL_CLASS = 'App\Models\Image';
    ...
    {
        return $this->morphMany(Tagged::class, 'refer');
    }
    ...
    public static function stores($input)
    {
        ...
        if (isset($input['tag'])) {
            $tags = implode('|', $input['tag']);
            $tag = $image->tagged()->create([
                'tags' => $tags,
            ]);
        }
    ...
    public static function updates($id, $input)
    {
        ...
        $tags['tags'] = implode('|', $input['tag']);
        $tag = Tagged::where('refer_id', $id)->update($tags);
