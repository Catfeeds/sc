...
class Moment extends BaseModule
{
    ...
    protected $fillable = ['id', 'member_id', 'content', 'topic', 'location', 'type', 'comment_num', 'like_num', 'state', 'created_at', 'updated_at'];
    ...
    public function scopeFilter($query, $filters)
    {
        $query->where(function ($query) use ($filters) {
            ...
            !empty($filters['member_id']) ? $query->where('member_id', $filters['member_id']) : '';
            empty($filters['last_id']) ?: $query->where('id', '<', $filters['last_id']);
            empty($filters['topic']) ?: $query->where('topic', $filters['topic']);
        });
    }
