...
-- -----------
-- 2018-5-18 10:00 Update completed.
-- -----------

-- -----------
-- 2018-5-18 yxy
-- -----------
ALTER TABLE `mk_course_views`
CHANGE COLUMN `member_id` `member_id` INT(10) UNSIGNED NOT NULL COMMENT '会员id' ,
CHANGE COLUMN `course_title` `course_title` VARCHAR(255) NOT NULL COMMENT '课程标题' ,
CHANGE COLUMN `chapter_seq` `chapter_seq` VARCHAR(20) NOT NULL COMMENT '章节序号（2-1）' ,
CHANGE COLUMN `lesson_title` `lesson_title` VARCHAR(255) NOT NULL COMMENT '课时标题' ,
CHANGE COLUMN `date` `date` TIMESTAMP NULL DEFAULT NULL COMMENT '观看日期（年月日）' ,
CHANGE COLUMN `updated_at` `updated_at` TIMESTAMP NULL DEFAULT NULL COMMENT '进度' ,
ADD COLUMN `progress` INT(10) UNSIGNED NOT NULL COMMENT '进度（秒）' AFTER `lesson_title`;


-- -----------
-- 2018-5-18 11:00 Update completed.
-- -----------

-- -----------
-- 2018-5-18 cb
-- -----------
ALTER TABLE `mk_lessons`
CHANGE COLUMN `duration` `duration` INT(10) UNSIGNED NOT NULL COMMENT '课程时长（单位秒）';
...
ALTER TABLE `mk_courses`
ADD COLUMN `video_url` VARCHAR(255) NOT NULL COMMENT '试看视频地址' AFTER `poster_url`;

ALTER TABLE `mk_courses`
CHANGE COLUMN `buy_num` `bought_num` INT(10) NOT NULL DEFAULT '0' COMMENT '购买人数' ;

-- -----------
-- 2018-5-18 18:00 Update completed.
-- -----------
...
-- -----------
ALTER TABLE `mk_members`
CHANGE COLUMN `state` `state` TINYINT(1) UNSIGNED NOT NULL COMMENT '状态 0已禁用 1启用' ;
-- -----------
-- 2018-5-19 yxy
-- -----------
ALTER TABLE `mk_course_views`
...
ALTER TABLE `mk_live_member`
CHANGE COLUMN `type` `state` TINYINT(1) UNSIGNED NOT NULL DEFAULT '1' COMMENT '状态 0已删除 1正常' AFTER `room_id`;

-- -----------
-- 2018-5-19 14:36 Update completed.
-- -----------
...
-- -----------
ALTER TABLE `mk_lives`
ADD COLUMN `room_ids` TEXT NULL DEFAULT NULL AFTER `bought_num`;


-- -----------
-- 2018-5-21 09:36 Update completed.
-- -----------
...
-- 2018-5-21 yxy
-- -----------
INSERT INTO `mk_options`(`code`,`name`,`value`,`type`,`option`) VALUES ('HOT_TEXT','热搜关键字(竖线“|”隔开)','素描|手绘|油画', 2, '');


-- -----------
-- 2018-5-21 18:01 Update completed.
-- -----------
...
CREATE TABLE `mk_certifications` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `member_id` INT(10) UNSIGNED NOT NULL COMMENT '申请教师id',
  `name` VARCHAR(50) NOT NULL COMMENT '真实姓名',
  `avatar_url` TEXT NOT NULL COMMENT '真实头像',
  `title` VARCHAR(255) NOT NULL COMMENT '职称（头衔）',
  `degree` VARCHAR(255) NOT NULL COMMENT '学历（学位）',
  `id_card` VARCHAR(20) NOT NULL COMMENT '身份证',
  `card_photo` VARCHAR(255) NOT NULL COMMENT '手持身份证照片',
  `reason` TEXT NOT NULL COMMENT '理由',
  `user_id` INT(10) UNSIGNED NOT NULL COMMENT '操作管理员id',
  `state` TINYINT(1) UNSIGNED NOT NULL COMMENT '状态（0未认证 1正在审核 2 审核通过 3审核失败）',
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  `deleted_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`id`));
...
ALTER TABLE `mk_articles`
DROP COLUMN `site_id`,
CHANGE COLUMN `published_at` `published_at` DATETIME NULL DEFAULT NULL COMMENT '发布时间' AFTER `state`,
CHANGE COLUMN `comment_num` `comment_num` INT(10) UNSIGNED NOT NULL COMMENT '评论数' ,
CHANGE COLUMN `view_num` `view_num` INT(10) UNSIGNED NOT NULL COMMENT '浏览数' ,
CHANGE COLUMN `state` `state` INT(10) UNSIGNED NOT NULL COMMENT '状态 0删除 1未发布 2撤回 9已发布' ;

-- -----------
-- 2018-5-22 yxy
-- -----------

CREATE TABLE `mk_coupons` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `refer_id` int(10) unsigned NOT NULL COMMENT '适用类型ID',
  `refer_type` varchar(64) NOT NULL COMMENT '适用类型',
  `code` varchar(255) NOT NULL COMMENT '优惠码',
  `type` tinyint(1) unsigned NOT NULL COMMENT '优惠方式（0打折 1抵扣）',
  `discount` float(10,2) unsigned NOT NULL COMMENT '若优惠方式为打折，则为打折率，若为抵价，则为抵价金额',
  `batch_id` int(10) unsigned NOT NULL COMMENT '批次号',
  `member_id` int(10) unsigned NOT NULL COMMENT '使用者',
  `order_id` int(10) unsigned NOT NULL COMMENT '订单号',
  `state` tinyint(1) unsigned NOT NULL COMMENT '状态 0未使用 1已使用 2失效',
  `use_at` timestamp NULL DEFAULT NULL COMMENT '使用时间',
  `deadline` timestamp NULL DEFAULT NULL COMMENT '失效时间',
...
-- -----------
-- 2018-5-23 16:40 Update completed.
-- -----------
...
-- -----------
ALTER TABLE `mk_order_log`
RENAME TO  `mk_order_logs` ;

-- -----------
-- 2018-5-24 09:15 Update completed.
-- -----------
...
ALTER TABLE `mk_coupons`
ADD COLUMN `user_id` INT(10) UNSIGNED NOT NULL COMMENT '创建人（管理员）' AFTER `order_id`;

-- -----------
-- 2018-5-24 12:45 Update completed.
-- -----------
