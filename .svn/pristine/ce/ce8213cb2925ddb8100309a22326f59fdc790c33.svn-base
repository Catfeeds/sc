...
class Chapter extends BaseModule
{
    ...
    const MODEL_CLASS = 'App\Models\Chapter';
    ...
    public static function stores($input)
    {
        ...
        $chapter = static::create($input);

        //保存图片集
        if (isset($input['images']) && !empty($input['images'])) {
            Item::sync(Item::TYPE_IMAGE, $chapter, $input['images']);
        }
    ...
    {
        return static::where('state', Chapter::STATE_PUBLISHED)->where('course_id', $id)->where('parent_id', 0)->orderBy('sort')->get();
    }
