...
class BaseController extends Controller
{
    ...
    {
        return trim(str_replace(["App\Api\Controllers", 'Controller'], "", $module), '\/');
    }

    public function enterClassLog($module, $member, $class, $function)
    {
        $this->log($module, '用户 ' . $member . ' 进入 ' . $class . ' 类的 ' . $function . '() 方法');
    }
