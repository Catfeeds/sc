...
function get_file_url($url)
{
    if (empty($url)) {
        ...
        return config('site.oss.host') . '/' . $url;
    }
}
...
/**
 * 根据token生成refresh_token，并返回token过期时间
 *
 * @return string
 */
function tokenTool($token)
{
    $payloadArr = explode('.', $token);
    $payload = $payloadArr[1];
    $payload = json_decode(base64_decode($payload), true);

    $data['expired_time'] = $payload['exp'];
    //+一个月生成刷新token
    $payload['exp'] = $payload['exp'] + 2592000;
    $new_payload = base64_encode(json_encode($payload));
    $data['refresh_token'] = $payloadArr[0] . '.' . $new_payload . '.' . $payloadArr[2];
