...
class CommentController extends BaseController
{
    ...
    public function lists()
    {
        ...
        if (!$module) {
            return $this->responseError('此类型不存在', 404);
        }

        $model = call_user_func([$module->model_class, 'find'], $id);

        if (empty($model)) {
            return $this->responseError('此ID不存在', 404);
        }
    ...
    public function delete()
    {
        ...
        if (empty($comment)) {
            return $this->responseError('评论不存在', 404);
        }
        //校验登录用户是否具有删除权限
        if ($comment->member_id !== $member->id) {
            return $this->responseError('您没有权限操作此评论', 403);
        }

        $result = $comment->delete();
        //对应模型的comment_num -1
        $model = call_user_func([$comment->refer_type, 'find'], $comment->refer_id);
        $model->decrement('comment_num');

        if ($result) {
            return $this->responseSuccess();
        } else {
            return $this->responseError('评论删除失败，请稍后再试');
        }
