...
class CourseController extends BaseController
{
    ...
    public function transform($course)
    {
        ...
        if ($member) {
            ...
            $courseView = $course->courseViews()->where('member_id', $member->id)->orderBy('id', 'desc')->first();
            if ($courseView) {
                $course->view_lesson_id = $courseView->lesson_id;
        ...
        if ($filter['sort'] == Course::SORT_TIME) {
            ...
            return cache_remember($key, 1, function () use ($filter, $order) {
                ...
                $courses = Course::where('state', course::STATE_PUBLISHED)
                    ->filter($filter)
                    ->select('id', 'title', 'member_id', 'subtitle', 'intro', 'is_free', 'price', 'bought_num', 'lesson_num', 'type', 'cover_url', 'poster_url', 'is_free', 'recommended', 'created_at', 'updated_at')
                    ->orderBy('id', $order)
                    ->limit($filter['limit'])
                    ->get();
            ...
            return cache_remember($key, 1, function () use ($filter, $order) {
                ...
                $courses = Course::where('state', course::STATE_PUBLISHED)
                    ->filter($filter)
                    ->select('id', 'title', 'subtitle', 'member_id', 'intro', 'is_free', 'price', 'bought_num', 'lesson_num', 'type', 'cover_url', 'poster_url', 'is_free', 'recommended', 'created_at', 'updated_at')
            ...
            return cache_remember($key, 1, function () use ($filter, $order) {
                ...
                $courses = DB::select("select *,($a*comment_num+$b*view_num+$c*bought_num) as hot_order from mk_courses where category_id = " . $filter['category_id'] . " order by hot_order desc limit " . ($filter['start'] - 1) * $filter['limit'] . "," . $filter['limit']);
            ...
            return cache_remember($key, 1, function () use ($filter, $order) {
                ...
                $courses = DB::select("select *,($a*comment_num+$b*view_num+$c*bought_num)/$c*(1+unix_timestamp(curdate())-unix_timestamp(DATE_FORMAT(created_at,'%Y-%m-%d'))) as hot_order from mk_courses where category_id = " . $filter['category_id'] . " order by hot_order desc limit " . ($filter['start'] - 1) * $filter['limit'] . "," . $filter['limit']);
    ...
    public function search()
    {
        ...
        $courses = Course::with('items')
            ...
            ->select('id', 'title', 'subtitle', 'price', 'comment_num', 'bought_num', 'lesson_num', 'type', 'cover_url', 'poster_url', 'created_at', 'updated_at')
            ->orderBy('sort', 'desc')
            ->orderBy('id', 'desc')
        ...
        foreach ($dates as $k => $date) {
            foreach ($lists as $key => $list) {
                if ($date['date'] == $list->date) {
                    $data[$k]['time'] = date('Y-m-d', strtotime($date['date']));
                    $data[$k]['list'][] = $list;
                }
            }
        }
